...
func (o *DescribeOptions) Run() string {
	r := o.NewBuilder().
		Unstructured().
		ContinueOnError().
	...
	if len(infos) == 0 && len(allErrs) == 0 {
		...
		if o.AllNamespaces {
			...
			var ret string
			ret = fmt.Sprintf(ret, "No resources found in %s namespace.\n", o.Namespace)
			return ret
		}
	}
...
func (o *DescribeOptions) DescribeMatchingResources(originalError error, resource, prefix string) string {
	r := o.NewBuilder().
		Unstructured().
